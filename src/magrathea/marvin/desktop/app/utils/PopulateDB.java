...
public class PopulateDB {
    ...
    private static final int INDEX_PRIZES = 0;
    ...
    private void query(MODEL model, int quantity) {
        switch (model) {
            case USERS: queryUsers(quantity); break;
            case TOURNAMENTS: queryTournaments(quantity); break;
            case PRIZES: queryPrizes(quantity); break;
            case GAMES: queryGames(quantity); break;
            case HOSTS: queryHosts(quantity); break;
        }
    }
    ...
    private static String[] GAMES_IMAGE = {"",
    ...
    private String getRoleUser(){
        ...
        if (adm != 1 ) editor = ( r.nextInt(100) < 10 ) ? 1 : 0 ; // 10%
        return adm + "','" + editor;
    }
    ...
    private String getGameName(){
        StringBuilder sb = new StringBuilder();
        if ( r.nextInt(2) == 0 ) {sb.append("The ");}
        sb.append( String.format( "%s %s%s", 
